...
- Diagram sources must be excluded from `code-features.js` and hidden synchronously before Mermaid loads; otherwise the raw definition flickers and receives an empty `.code-wrapper`. Remove Mermaid's generated inline `max-width` and derive one stable widget ratio from the narrowest rendered step viewBox. The 26rem-wide stage is bounded to 6rem–18rem high, so square BFS uses 288px while horizontal Bubble Sort uses 96px without resizing between steps. Microsoft Edge 150 confirmed drawings of 262×264px and 401×51px respectively.
- Verify YAML and static markup with `StaticGenerationTests` and `BlogPageTests`; verify controls, eager rendering, stable aspect metadata, error fallback, single-step behavior, and theme refresh with `interactive-diagrams.test.js`. `interactive-diagrams.spec.js` is the browser regression for compact aspect-aware sizing, pre-rendered steps, and a stable Next transition.
