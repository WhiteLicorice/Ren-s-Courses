...
- Diagram sources must be excluded from `code-features.js` and hidden synchronously before Mermaid loads or a step changes; otherwise the raw definition flickers and receives an empty `.code-wrapper`. Remove Mermaid's generated inline `max-width` after rendering and let `.diagram-canvas svg` use the canvas width with a 32rem/70vh height cap. This was confirmed from live DOM measurements and desktop/mobile Chromium screenshots.
